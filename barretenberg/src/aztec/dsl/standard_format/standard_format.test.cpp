...
TEST(standard_format, pedersen_constraint)
{
    ...
    fr x = fr::zero();
    fr y = fr::one();
    auto res = crypto::pedersen::encrypt_native({ x, y });
...
TEST(standard_format, ecdsa_verify_constraint)
{
    ...
    crypto::ecdsa::signature signature =
        crypto::ecdsa::construct_signature<Blake2sHasher, secp256k1::fq, secp256k1::fr, secp256k1::g1>(message_string,
                                                                                                       account);
    ...
    std::vector<fr> witness_values;
    std::copy(hashed_message_bytes_as_fr.begin(), hashed_message_bytes_as_fr.end(), std::back_inserter(witness_values));
    // witness_values.emplace_back(public_key_x);
    // witness_values.emplace_back(public_key_y);
    witness_values.emplace_back(fr::zero());
